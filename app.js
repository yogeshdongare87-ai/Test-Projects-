...
map.on('pm:create', (e) => {
    ...
    if (e.shape === 'Polygon' || e.shape === 'Rectangle') {
        ...
        infoPanel.innerHTML = `
            <h2>📐 Measured Manual Area</h2>
            <div class="info-row"><span class="info-label">Sq. Meters</span><span class="info-value">${areaSqMeters.toFixed(2)} m²</span></div>
            <div class="info-row"><span class="info-label">Hectares</span><span class="info-value">${areaHectares} Ha</span></div>
            <div class="info-row"><span class="info-label">Acres</span><span class="info-value">${areaAcres} Acres</span></div>
        `;
    }
});
...
// =====================================================
// SEARCH FUNCTIONALITY (STRICTLY SEARCHES ONLY "Text")
// =====================================================
    ...
    geojsonInput.addEventListener("change", function (e) {
        ...
        reader.onload = function (event) {
            try {
                ...
                const tempPolyData = polyFeatures.length > 0 ? { type: "FeatureCollection", features: polyFeatures } : null;
                const tempLineData = lineFeatures.length > 0 ? { type: "FeatureCollection", features: lineFeatures } : null;

                if (tempPolyData) {
                    polygonLayer = L.geoJSON(tempPolyData, {
                        style: { color: "#4f46e5", weight: 1.5, fillColor: "#818cf8", fillOpacity: 0.35 },
                        onEachFeature: (feature, layer) => setupFeatureEvents(feature, layer, 'polygon')
                    }).addTo(map);
                }
                ...
                if (tempLineData) {
                    lineLayer = L.geoJSON(tempLineData, {
                        style: (feature) => getRoadLineStyle(feature),
                        onEachFeature: (feature, layer) => setupFeatureEvents(feature, layer, 'line')
                    }).addTo(map);
                }
                ...
                generateVillageReport(tempPolyData, tempLineData);
